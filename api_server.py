...
<div class="page" id="page-admin">
  ...
  <div class="tc" id="at-cli">
    ...
    <!-- ══ SERVER HEALTH STATUS (Admin CLI tab) ══ -->
    <div class="card" style="margin-bottom:16px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;flex-wrap:wrap;gap:10px">
        <div>
          ...
          <div style="font-size:10px;color:var(--m);font-family:var(--font-mono)">
            ...
            <span id="adm-health-last-updated" style="color:var(--cyan)">loading...</span>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:10px">
          <span class="pulse-dot" id="adm-health-pulse"></span>
          <span id="adm-health-status-label" style="font-size:11px;font-family:var(--font-mono);color:var(--green)">ONLINE</span>
          <button onclick="admLoadHealthStatus()"
      ...
      <div class="health-grid">
        <div class="health-card">
          ...
          <div class="health-card-val" id="adm-hc-cpu" style="color:var(--cyan)">—</div>
          <div class="health-card-sub" id="adm-hc-cpu-sub">Loading...</div>
          <div class="health-bar"><div class="health-bar-fill" id="adm-hc-cpu-bar" style="background:var(--cyan);width:0%"></div></div>
        </div>
        <div class="health-card">
          <div class="health-card-icon">🧠</div>
          <div class="health-card-label">MEMORY (RAM)</div>
          <div class="health-card-val" id="adm-hc-mem" style="color:var(--purple)">—</div>
          <div class="health-card-sub" id="adm-hc-mem-sub">Loading...</div>
          <div class="health-bar"><div class="health-bar-fill" id="adm-hc-mem-bar" style="background:var(--purple);width:0%"></div></div>
        </div>
        <div class="health-card">
          <div class="health-card-icon">💾</div>
          <div class="health-card-label">DISK STORAGE</div>
          <div class="health-card-val" id="adm-hc-disk" style="color:var(--yellow)">—</div>
          <div class="health-card-sub" id="adm-hc-disk-sub">Loading...</div>
          <div class="health-bar"><div class="health-bar-fill" id="adm-hc-disk-bar" style="background:var(--yellow);width:0%"></div></div>
        </div>
        <div class="health-card">
          <div class="health-card-icon">🌐</div>
          <div class="health-card-label">PUBLIC IP</div>
          <div class="health-card-val" id="adm-hc-ip" style="color:var(--green);font-size:16px;margin-top:6px">—</div>
          <div class="health-card-sub" id="adm-hc-ip-sub">Fetching...</div>
        </div>
        <div class="health-card">
          <div class="health-card-icon">⏱️</div>
          <div class="health-card-label">UPTIME</div>
          <div class="health-card-val" id="adm-hc-uptime" style="color:var(--orange);font-size:18px;margin-top:4px">—</div>
          <div class="health-card-sub" id="adm-hc-uptime-sub">Boot time</div>
        </div>
        <div class="health-card">
          <div class="health-card-icon">⚙️</div>
          <div class="health-card-label">PROCESSES</div>
          <div class="health-card-val" id="adm-hc-procs" style="color:var(--red)">—</div>
          <div class="health-card-sub" id="adm-hc-procs-sub">Running / Total</div>
        </div>
        <div class="health-card">
          <div class="health-card-icon">🌡️</div>
          <div class="health-card-label">LOAD AVERAGE</div>
          <div class="health-card-val" id="adm-hc-load" style="color:var(--cyan);font-size:15px;margin-top:6px">—</div>
          <div class="health-card-sub" id="adm-hc-load-sub">1m / 5m / 15m</div>
        </div>
        <div class="health-card">
          <div class="health-card-icon">🔌</div>
          <div class="health-card-label">NETWORK I/O</div>
          <div class="health-card-val" id="adm-hc-net" style="color:var(--green);font-size:14px;margin-top:6px">—</div>
          <div class="health-card-sub" id="adm-hc-net-sub">↑ Sent / ↓ Received</div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));
        ...
        <div style="font-family:var(--font-mono);font-size:11px"><span style="color:var(--m)">HOSTNAME:</span> <span id="adm-hc-hostname" style="color:var(--t)">—</span></div>
        <div style="font-family:var(--font-mono);font-size:11px"><span style="color:var(--m)">OS:</span> <span id="adm-hc-os" style="color:var(--t)">—</span></div>
        <div style="font-family:var(--font-mono);font-size:11px"><span style="color:var(--m)">KERNEL:</span> <span id="adm-hc-kernel" style="color:var(--t)">—</span></div>
        <div style="font-family:var(--font-mono);font-size:11px"><span style="color:var(--m)">PYTHON:</span> <span id="adm-hc-python" style="color:var(--cyan)">—</span></div>
        <div style="font-family:var(--font-mono);font-size:11px"><span style="color:var(--m)">LOCAL IP:</span> <span id="adm-hc-localip" style="color:var(--t)">—</span></div>
        <div style="font-family:var(--font-mono);font-size:11px"><span style="color:var(--m)">LISTENING PORTS:</span> <span id="adm-hc-openports" style="color:var(--yellow)">—</span></div>
      </div>
    </div><!-- /health card -->

    <!-- ══ LIVE STREAMING TERMINAL (Admin CLI tab) ══ -->
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:10px">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <span class="pulse-dot" id="adm-console-status-dot"></span>
          <span id="adm-console-status-txt" style="font-size:10px;color:var(--green);font-family:var(--font-mono)">READY</span>
          <button onclick="admConsoleClear()"
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px">
        <button class="quick-pill" onclick="admConsoleQuick('uptime')">uptime</button>
        <button class="quick-pill" onclick="admConsoleQuick('df -h')">df -h</button>
        <button class="quick-pill" onclick="admConsoleQuick('free -h')">free -h</button>
        <button class="quick-pill" onclick="admConsoleQuick('ps aux --sort=-%cpu | head -15')">top procs</button>
        <button class="quick-pill" onclick="admConsoleQuick('ss -tlnp')">open ports</button>
        <button class="quick-pill" onclick="admConsoleQuick('ip addr')">ip addr</button>
        <button class="quick-pill" onclick="admConsoleQuick('uname -a')">uname</button>
        <button class="quick-pill" onclick="admConsoleQuick('last -n 10')">last logins</button>
        <button class="quick-pill" onclick="admConsoleQuick('journalctl -n 30 --no-pager')">recent logs</button>
        <button class="quick-pill" onclick="admConsoleQuick('cat /etc/os-release')">os info</button>
        <button class="quick-pill" onclick="admConsoleQuick('which nmap nikto lynis dnsrecon theHarvester wpscan')">check tools</button>
        <button class="quick-pill" onclick="admConsoleQuick('systemctl status vulnscan 2>&1 | head -20')">svc status</button>
        <button class="quick-pill" onclick="admConsoleQuick('help')">help</button>
      </div>

      <!-- Terminal window -->
      <div class="live-terminal" id="adm-live-terminal">
        <div class="live-terminal-header">
          ...
          <span style="font-family:var(--font-mono);font-size:11px;color:var(--m);margin-left:8px"
                id="adm-terminal-title">vulnscan@server — admin console</span>
          <span style="margin-left:auto;font-size:10px;color:var(--m);font-family:var(--font-mono)"
                id="adm-terminal-cmd-count">0 commands run</span>
        </div>
        <div class="live-terminal-output" id="adm-live-term-output">
          <div class="term-line sys">VulnScan Pro — Admin Live Console v3.7</div>
          ...
          <div class="term-line sys" style="border-bottom:1px solid var(--b);padding-bottom:8px;margin-bottom:8px">─────────────────────────────────────────────────────────</div>
        </div>
        <div class="live-terminal-input">
          <span class="term-prompt" id="adm-term-prompt-user">admin@vulnscan:~$</span>
          <input class="term-input" id="adm-live-term-input"
                 type="text" placeholder="enter command..."
                 autocomplete="off" spellcheck="false"
                 onkeydown="admLiveTermKey(event)"
                 oninput="admLiveTermHint(this.value)"/>
          <span id="adm-live-term-hint"
          ...
          <button class="term-run-btn" id="adm-term-run-btn" onclick="admLiveTermRun()">▶ RUN</button>
          <button id="adm-term-cancel-btn" onclick="admLiveTermCancel()"
...
function adminTab(e,id){
  ...
  if(id==="at-cli"){
    admLoadHealthStatus();
    admStartHealthPolling();
    if(currentUser){var _p=document.getElementById('adm-term-prompt-user');if(_p)_p.textContent=(currentUser.username||'admin')+'@vulnscan:~$';}
    setTimeout(function(){var _i=document.getElementById('adm-live-term-input');if(_i)_i.focus();},200);
  } else {
    admStopHealthPolling();
  }
}
...
// ════════════════════════════════════════════════════════════════
//  ADMIN CLI TAB — Health polling + Live streaming terminal
// ════════════════════════════════════════════════════════════════

let _admHealthTimer = null;

function admStartHealthPolling() {
  admStopHealthPolling();
  _admHealthTimer = setInterval(admLoadHealthStatus, 10000);
}
function admStopHealthPolling() {
  if (_admHealthTimer) { clearInterval(_admHealthTimer); _admHealthTimer = null; }
}

async function admLoadHealthStatus() {
  try {
    const r = await fetch('/api/health-status');
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const d = await r.json();
    ...
    const lbl = document.getElementById('adm-health-status-label');
    if (lbl) { lbl.textContent = 'ERROR'; lbl.style.color = 'var(--red)'; }
    const upd = document.getElementById('adm-health-last-updated');
    if (upd) upd.textContent = 'fetch failed — ' + e.message;
  }
}

function _admSetEl(id, text) {
  const el = document.getElementById(id);
  if (el) el.textContent = text;
}
function _admBar(id, pct, color) {
  const el = document.getElementById(id);
  if (el) { el.style.width = Math.min(pct, 100) + '%'; el.style.background = color; }
}

function _admRenderHealth(d) {
  const upd = document.getElementById('adm-health-last-updated');
  if (upd) upd.textContent = 'updated ' + new Date().toLocaleTimeString();

  const cpuPct = d.cpu_percent || 0;
  _admSetEl('adm-hc-cpu', cpuPct.toFixed(1) + '%');
  _admSetEl('adm-hc-cpu-sub', `${d.cpu_count || '?'} logical cores`);
  _admBar('adm-hc-cpu-bar', cpuPct, _clampColor(cpuPct));

  const mem = d.memory || {};
  _admSetEl('adm-hc-mem', (mem.percent || 0).toFixed(1) + '%');
  _admSetEl('adm-hc-mem-sub', `${mem.used_gb || 0} GB / ${mem.total_gb || 0} GB`);
  _admBar('adm-hc-mem-bar', mem.percent || 0, _clampColor(mem.percent || 0));

  const disk = d.disk || {};
  _admSetEl('adm-hc-disk', (disk.percent || 0).toFixed(1) + '%');
  _admSetEl('adm-hc-disk-sub', `${disk.used_gb || 0} GB / ${disk.total_gb || 0} GB`);
  _admBar('adm-hc-disk-bar', disk.percent || 0, _clampColor(disk.percent || 0));

  _admSetEl('adm-hc-ip', d.public_ip || '—');
  _admSetEl('adm-hc-ip-sub', d.public_ip === 'unavailable' ? 'No external access' : 'External address');

  const up = d.uptime || {};
  _admSetEl('adm-hc-uptime', up.human || '—');
  _admSetEl('adm-hc-uptime-sub', up.boot_time ? 'Boot: ' + up.boot_time : '');

  const procs = d.processes || {};
  _admSetEl('adm-hc-procs', procs.total || '—');
  _admSetEl('adm-hc-procs-sub', `${procs.running || 0} running / ${procs.total || 0} total`);

  const load = d.load_avg || {};
  _admSetEl('adm-hc-load', `${load['1m']||0} · ${load['5m']||0} · ${load['15m']||0}`);

  const net = d.network || {};
  _admSetEl('adm-hc-net', `↑ ${net.sent || '—'}  ↓ ${net.received || '—'}`);

  const sys = d.system || {};
  _admSetEl('adm-hc-hostname', sys.hostname || '—');
  _admSetEl('adm-hc-os', sys.os || '—');
  _admSetEl('adm-hc-kernel', (sys.kernel || '—').slice(0, 55));
  _admSetEl('adm-hc-python', sys.python || '—');
  _admSetEl('adm-hc-localip', d.local_ip || '—');
  const ports = (d.open_ports || []).slice(0, 12).join(', ');
  _admSetEl('adm-hc-openports', ports || '—');
...
// ── Admin live terminal state ──
let _admTermHistory  = [];
let _admTermHistIdx  = -1;
let _admTermRunning  = false;
let _admTermCmdCount = 0;
let _admActiveReader = null;
...
function admLiveTermHint(val) {
  const hint = document.getElementById('adm-live-term-hint');
  if (!hint) return;
  if (!val) { hint.textContent = ''; return; }
  const match = Object.entries(ADM_HINT_MAP).find(([k]) => k.startsWith(val.toLowerCase()));
  hint.textContent = match ? match[1].slice(val.length) : '';
}

function admLiveTermKey(e) {
  const inp = document.getElementById('adm-live-term-input');
  if (!inp) return;
  if (e.key === 'Enter') { e.preventDefault(); admLiveTermRun(); return; }
  if (e.key === 'Tab') {
    e.preventDefault();
    const hint = document.getElementById('adm-live-term-hint');
    if (hint && hint.textContent) { inp.value += hint.textContent; hint.textContent = ''; }
    return;
  }
  if (e.key === 'ArrowUp') {
    e.preventDefault();
    if (_admTermHistIdx < _admTermHistory.length - 1) {
      _admTermHistIdx++;
      inp.value = _admTermHistory[_admTermHistory.length - 1 - _admTermHistIdx] || '';
      admLiveTermHint(inp.value);
    }
    return;
  }
  if (e.key === 'ArrowDown') {
    e.preventDefault();
    if (_admTermHistIdx > 0) { _admTermHistIdx--; inp.value = _admTermHistory[_admTermHistory.length - 1 - _admTermHistIdx] || ''; }
    else { _admTermHistIdx = -1; inp.value = ''; }
    admLiveTermHint(inp.value);
    return;
  }
}

function admConsoleQuick(cmd) {
  const inp = document.getElementById('adm-live-term-input');
  if (inp) { inp.value = cmd; inp.focus(); admLiveTermHint(cmd); }
  admLiveTermRun();
}

function admConsoleClear() {
  const out = document.getElementById('adm-live-term-output');
  if (!out) return;
  out.innerHTML = '';
  _admTermAddLine('Console cleared.', 'sys');
  _admTermCmdCount = 0;
  _admUpdateCmdCount();
}

function admLiveTermCancel() {
  if (_admActiveReader) { _admActiveReader.cancel(); _admActiveReader = null; }
  _admSetTermRunning(false);
  _admTermAddLine('\n[Stream cancelled by user]', 'sys');
}

async function admLiveTermRun() {
  const inp = document.getElementById('adm-live-term-input');
  if (!inp) return;
  const cmd = inp.value.trim();
  if (!cmd || _admTermRunning) return;

  if (_admTermHistory[_admTermHistory.length - 1] !== cmd) _admTermHistory.push(cmd);
  if (_admTermHistory.length > 200) _admTermHistory.shift();
  _admTermHistIdx = -1;
  inp.value = '';
  document.getElementById('adm-live-term-hint').textContent = '';

  _admTermAddLine('$ ' + cmd, 'cmd');
  _admSetTermRunning(true);
  _admTermCmdCount++;
  _admUpdateCmdCount();
  ...
  try {
    ...
    if (!res.ok) { _admTermAddLine('HTTP error ' + res.status, 'err'); _admSetTermRunning(false); return; }

    const reader = res.body.getReader();
    _admActiveReader = reader;
    const dec = new TextDecoder();
    let buf = '';
    ...
    while (true) {
      ...
      for (const part of parts) {
        ...
        try {
          ...
          if (obj.done) {
            const code = obj.exit_code || 0;
            _admTermAddLine(code === 0 ? '✓ Done (exit 0)' : `✗ Exit code ${code}`, code === 0 ? 'ok' : 'err');
          } else if (obj.line !== undefined) {
            _admTermAddLine(obj.line, obj.type || 'out');
          }
        } catch (_) {}
      }
    }
  } catch (e) {
    if (e.name !== 'AbortError') _admTermAddLine('Error: ' + e.message, 'err');
  } finally {
    _admActiveReader = null;
    _admSetTermRunning(false);
    _admScrollTermToBottom();
  }
}

function _admTermAddLine(text, cls = 'out') {
  const out = document.getElementById('adm-live-term-output');
  if (!out) return;
  const div = document.createElement('div');
  div.className = 'term-line ' + cls;
...
function _admScrollTermToBottom() {
  const out = document.getElementById('adm-live-term-output');
  if (out) requestAnimationFrame(() => { out.scrollTop = out.scrollHeight; });
}

function _admSetTermRunning(running) {
  _admTermRunning = running;
  const btn    = document.getElementById('adm-term-run-btn');
  const cancel = document.getElementById('adm-term-cancel-btn');
  const inp    = document.getElementById('adm-live-term-input');
  const status = document.getElementById('adm-console-status-txt');
  if (btn)    { btn.disabled = running; btn.textContent = running ? '⏳' : '▶ RUN'; }
  if (cancel) cancel.style.display = running ? 'inline-block' : 'none';
  if (inp)    inp.disabled = running;
  if (status) { status.textContent = running ? 'RUNNING...' : 'READY'; status.style.color = running ? 'var(--yellow)' : 'var(--green)'; }
...
function _admUpdateCmdCount() {
  const el = document.getElementById('adm-terminal-cmd-count');
  if (el) el.textContent = _admTermCmdCount + ' command' + (_admTermCmdCount === 1 ? '' : 's') + ' run';
}

// Also stop admin health polling when leaving admin page entirely
const _origPg = pg;
// Patch pg to stop adm health polling when navigating away from admin
(function(){
  const _pg = pg;
  window.pg = function(id, el) {
    if (id !== 'admin') admStopHealthPolling();
    _pg(id, el);
  };
})();
...
def dnsrecon_route():
    ...
    if not binary:
        ...
        if not ok:
            return jsonify({"error": f"dnsrecon not installed and auto-install failed: {msg}. Run: sudo apt install dnsrecon", "auto_install_attempted": True})
        binary = shutil.which("dnsrecon")
    import tempfile
...
def nikto_route():
    ...
    if not binary:
        ...
        if not ok:
            return jsonify({"error": f"Nikto not installed and auto-install failed: {msg}. Run: sudo apt install nikto", "auto_install_attempted": True})
        binary = shutil.which("nikto")
    import tempfile
...
def lynis_route():
    ...
    if not binary:
        ...
        if not ok:
            return jsonify({"error": f"Lynis not installed and auto-install failed: {msg}. Run: sudo apt install lynis", "auto_install_attempted": True})
        binary = shutil.which("lynis")
        if not binary:
            return jsonify({"error": "Auto-install ran but lynis still not found. Run: sudo apt install lynis"})
...
def harvester():
    ...
    if not shutil.which("theHarvester") and not shutil.which("theharvester"):
        ...
        if not ok:
            return jsonify({"error": f"theHarvester not installed and auto-install failed: {msg}. Run: sudo apt install theharvester", "auto_install_attempted": True})
...
def auto_install(pkg, binary=None):
    """Try to install a package via apt, return (success, message)"""
...
# Tool install map: tool_name -> (apt_pkg, binary)
TOOL_INSTALL_MAP = {
    "nmap":         ("nmap",         "nmap"),
    "nikto":        ("nikto",        "nikto"),
    "lynis":        ("lynis",        "lynis"),
    ...
    "wpscan":       (None,           "wpscan"),   # gem-based, skip
    "dig":          ("dnsutils",     "dig"),
}
...
# ── Global theme storage (server-side, applies to all users) ──────────────────
...
def theme_api():
    ...
    if request.method == "POST":
        data = request.get_json() or {}
        t = data.get("theme", "cyberpunk")
        _global_theme["theme"] = t
